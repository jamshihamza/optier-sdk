...
for ptype in ["AlarmConfig", "ChannelConfig", "AllConfig", None]:
    ...
    print("=" * 60)
    print(f"Cross Counting Range ({label})")
    print("=" * 60)
    try:
        rng = cam.cross_counting.range(page_type=ptype)
        ...
        print(f"Cross Counting Range ({label}) error: {exc}")

    print()
    print("=" * 60)
    print(f"Cross Counting Get ({label})")
    print("=" * 60)
    try:
        cfg = cam.cross_counting.get(page_type=ptype)
        ...
        print(f"Configured/Active Cross Counting channels ({len(active_channels)}): {list(active_channels.keys())}")
        ...
        print(f"Cross Counting Get ({label}) error: {exc}")
    print()
