...
for ptype in ["AlarmConfig", "ChannelConfig", "AllConfig", None]:
    ...
    print("=" * 60)
    print(f"Occlusion Alarm Range ({label})")
    print("=" * 60)
    try:
        rng = cam.occlusion_alarm.range(page_type=ptype)
        ...
        print(f"Occlusion Alarm Range ({label}) error: {exc}")

    print()
    print("=" * 60)
    print(f"Occlusion Alarm Get ({label})")
    print("=" * 60)
    try:
        cfg = cam.occlusion_alarm.get(page_type=ptype)
        ...
        print(f"Configured/Active Occlusion channels ({len(active_channels)}): {list(active_channels.keys())}")
        ...
        print(f"Occlusion Alarm Get ({label}) error: {exc}")
    print()
