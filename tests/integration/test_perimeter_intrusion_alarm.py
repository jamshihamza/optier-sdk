...
for ptype in ["AlarmConfig", "ChannelConfig", "AllConfig", None]:
    ...
    print("=" * 60)
    print(f"Perimeter Intrusion Alarm Range ({label})")
    print("=" * 60)
    try:
        rng = cam.perimeter_intrusion_alarm.range(page_type=ptype)
        ...
        print(f"Perimeter Intrusion Alarm Range ({label}) error: {exc}")

    print()
    print("=" * 60)
    print(f"Perimeter Intrusion Alarm Get ({label})")
    print("=" * 60)
    try:
        cfg = cam.perimeter_intrusion_alarm.get(page_type=ptype)
        ...
        print(f"Configured/Active PID channels ({len(active_channels)}): {list(active_channels.keys())}")
        ...
        print(f"Perimeter Intrusion Alarm Get ({label}) error: {exc}")
    print()
