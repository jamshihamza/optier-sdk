...
for ptype in ["AlarmConfig", "ChannelConfig", "AllConfig", None]:
    ...
    print("=" * 60)
    print(f"Face Detection Range ({label})")
    print("=" * 60)
    try:
        rng = cam.face_detection.range(page_type=ptype)
        ...
        print(f"Face Detection Range ({label}) error: {exc}")

    print()
    print("=" * 60)
    print(f"Face Detection Get ({label})")
    print("=" * 60)
    try:
        cfg = cam.face_detection.get(page_type=ptype)
        ...
        print(f"Configured/Active Face Detection channels ({len(active_channels)}): {list(active_channels.keys())}")
        ...
        print(f"Face Detection Get ({label}) error: {exc}")
    print()
