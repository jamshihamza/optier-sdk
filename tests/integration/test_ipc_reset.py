...
print("=" * 60)
print("Maintenance IPC Reset (Load Default) Range")
print("=" * 60)
try:
    range_res = cam.ipc_reset.range()
    channels = list(range_res.get("channel_info", {}).get("items", {}).keys())
    print(f"IPC Reset Configured Channel Slots: {len(channels)}")
except OptierSDKError as exc:
    print(f"IPCReset Range error: {exc}")

print()
print("=" * 60)
print("Maintenance IPC Reset (Load Default) Get")
print("=" * 60)
try:
    get_res = cam.ipc_reset.get()
    channels = get_res.get("channel_info", {})
    print(f"IPC Reset Total Channels: {len(channels)}")
    ...
    print(f"IPCReset Get error: {exc}")
