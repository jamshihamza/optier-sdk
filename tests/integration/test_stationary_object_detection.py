...
for ptype in ["AlarmConfig", "ChannelConfig", "AllConfig", None]:
    ...
    print("=" * 60)
    print(f"Stationary Object Detection Range ({label})")
    print("=" * 60)
    try:
        rng = cam.stationary_object_detection.range(page_type=ptype)
        ...
        print(f"Stationary Object Detection Range ({label}) error: {exc}")

    print()
    print("=" * 60)
    print(f"Stationary Object Detection Get ({label})")
    print("=" * 60)
    try:
        cfg = cam.stationary_object_detection.get(page_type=ptype)
        ...
        unsupported_channels = [
            ...
            if isinstance(v, dict) and v.get("status") in ["Nonsupport", "NotSupport", "Notsupport"]
        ]
        unconfigured_channels = [k for k, v in ch_info.items() if isinstance(v, dict) and v.get("reason") == "Not configured"]

        print(f"Configured/Active Stationary Object Detection channels ({len(active_channels)}): {list(active_channels.keys())}")
        ...
        print(f"Stationary Object Detection Get ({label}) error: {exc}")
    print()
