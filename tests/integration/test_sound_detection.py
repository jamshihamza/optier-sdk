...
for ptype in ["AlarmConfig", "ChannelConfig", "AllConfig", None]:
    ...
    print("=" * 60)
    print(f"Sound Detection Range ({label})")
    print("=" * 60)
    try:
        rng = cam.sound_detection.range(page_type=ptype)
        ...
        print(f"Sound Detection Range ({label}) error: {exc}")

    print()
    print("=" * 60)
    print(f"Sound Detection Get ({label})")
    print("=" * 60)
    try:
        cfg = cam.sound_detection.get(page_type=ptype)
        ...
        print(f"Configured/Active Sound Detection channels ({len(active_channels)}): {list(active_channels.keys())}")
        ...
        for ch, ch_data in list(active_channels.items())[:3]:
            ...
            summary = {
                ...
                if k not in ["voice_prompts_time", "voice_prompts_index", "time_schedule"]
            }
            pprint.pprint(summary, sort_dicts=False)
    except OptierSDKError as exc:
        print(f"Sound Detection Get ({label}) error: {exc}")
    print()
