...
for ptype in ["AlarmConfig", "ChannelConfig", "AllConfig", None]:
    ...
    print("=" * 60)
    print(f"Pedestrian Detection Range ({label})")
    print("=" * 60)
    try:
        rng = cam.pedestrian_detection.range(page_type=ptype)
        ...
        print(f"Pedestrian Detection Range ({label}) error: {exc}")

    print()
    print("=" * 60)
    print(f"Pedestrian Detection Get ({label})")
    print("=" * 60)
    try:
        cfg = cam.pedestrian_detection.get(page_type=ptype)
        ...
        unsupported_channels = [
            ...
            if isinstance(v, dict) and v.get("status") in ["Nonsupport", "NotSupport"]
        ]
        unconfigured_channels = [k for k, v in ch_info.items() if isinstance(v, dict) and v.get("reason") == "Not configured"]

        print(f"Configured/Active Pedestrian Detection channels ({len(active_channels)}): {list(active_channels.keys())}")
        ...
        print(f"Pedestrian Detection Get ({label}) error: {exc}")
    print()
